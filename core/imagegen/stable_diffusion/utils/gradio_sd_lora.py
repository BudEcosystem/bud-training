...
def test_stable_diffusion_t2i(
    ...
    images = pipe(
        ...
        generator=torch.manual_seed(seed) if seed != -1 else torch.manual_seed(random.randint(0, 3452029))
    ).images
...
with gr.Blocks() as demo:
    with gr.Row():
        with gr.Group():
            ...
            guidance=gr.Slider(minimum=0, maximum=1, value=0.7,step=0.01, label="Guidance Scale")
            btn = gr.Button(value="Submit")
        outputs=gr.Image(type="pil", label="Generated Image")

    btn.click(test_stable_diffusion_t2i, inputs=[prompt, negative_prompt, guidance], outputs=[outputs])
